...
export function seedJavaQuestions() {
  ...
  const qs = [
    ...
    { category: 'Java基础', question: 'final、finally、finalize区别？', answer: 'final修饰类不可继承、方法不可重写、变量不可重赋值。finally异常处理保证执行。finalize在GC前调用(JDK9+废弃)。', tags: 'final,finally', importance: 3 },
    ...
    { category: 'JVM', question: 'JVM调优常用参数？', answer: '-Xms/-Xmx堆大小、-Xss栈大小、-XX:NewRatio新生代比例、-XX:+UseG1GC垃圾回收器、-XX:+PrintGCDetails日志、-XX:MetaspaceSize元空间。', tags: 'JVM,调优', importance: 3 },
    ...
    { category: '设计模式', question: '工厂模式和抽象工厂区别？', answer: '工厂方法：一个工厂生产一种产品。抽象工厂：一个工厂生产一族产品。简单工厂不属于GoF23种模式。', tags: '工厂,设计模式', importance: 3 },
    { category: '设计模式', question: '观察者模式和发布订阅区别？', answer: '观察者：观察者和被观察者直接耦合(同步)。发布订阅：通过中间消息队列解耦(异步)。发布订阅是观察者模式的进化版。', tags: '观察者,发布订阅', importance: 3 },
    ...
    { category: '消息队列', question: '如何保证消息顺序消费？', answer: 'RocketMQ：相同业务key路由到同一队列。Kafka：相同key路由到同一partition。单partition单线程消费保证顺序。', tags: 'MQ,顺序消费', importance: 3 },
    ...
    { category: 'Java基础', question: 'Java SPI机制是什么？', answer: 'Service Provider Interface，服务提供者可插拔机制。定义接口+META-INF/services/配置文件+ServiceLoader加载。打破双亲委派。应用：JDBC驱动、Dubbo扩展。', tags: 'SPI,可插拔', importance: 3 },
    ...
    { category: '并发编程', question: 'CountDownLatch和CyclicBarrier区别？', answer: 'CountDownLatch：等待其他线程完成，不可重置。CyclicBarrier：互相等待到共同屏障点，可重置，支持barrierAction。CountDownLatch是等人做完，CyclicBarrier是等人到齐。', tags: '并发工具', importance: 3 },
    ...
    { category: 'Java基础', question: 'Java 8-17新特性？', answer: 'Java8: Lambda/Stream/Optional/新日期API。Java9: 模块化/jshell。Java11: HttpClient/var。Java14: switch表达式/record。Java16: Pattern Matching。Java17: sealed class/text block。', tags: '新特性,版本', importance: 3 },
    ...
    { category: '设计模式', question: '策略模式和模板方法区别？', answer: '策略模式：封装算法族，运行时选择(组合)。模板方法：定义算法骨架，子类重写步骤(继承)。策略更灵活可运行时切换，模板适合固定流程。', tags: '策略,模板方法', importance: 3 },
...
export function seedAIQuestions() {
  ...
  const qs = [
    ...
    { category: 'LLM基础', question: 'Temperature和Top-P的作用？', answer: 'Temperature：控制输出随机性，低(0.1)确定性高，高(1.0)更随机。Top-P(Nucleus Sampling)：从累计概率达到P的token中采样。两者配合使用控制生成多样性。', tags: '采样,生成控制', importance: 4 },
    ...
    { category: 'RAG', question: 'RAG中文档切分策略？', answer: '固定大小切分(按token数)、语义切分(按段落/章节)、递归切分(LangChain RecursiveCharacterTextSplitter)。关键参数：chunk_size(512-1024)、chunk_overlap(50-200)。需考虑上下文完整性。', tags: 'Chunking,文档处理', importance: 4 },
    { category: 'RAG', question: '向量数据库有哪些？怎么选？', answer: 'Milvus(开源分布式)、Pinecone(全托管)、Weaviate(图搜索)、Chroma(轻量本地)、Qdrant(Rust高性能)、FAISS(Meta开源库)。选型考虑：规模、延迟、成本、部署方式。', tags: '向量数据库,Embedding', importance: 4 },
    ...
    { category: 'Agent框架', question: 'ReAct模式是什么？', answer: 'Reasoning + Acting。Agent交替进行推理(Thought)和行动(Action)，观察结果(Observation)后继续推理。形成Thought→Action→Observation循环直到得出最终答案。', tags: 'ReAct,推理行动', importance: 4 },
    ...
    { category: '记忆系统', question: '如何解决LLM上下文窗口限制？', answer: '1.摘要压缩(长对话压缩为摘要) 2.滑动窗口(只保留最近N轮) 3.RAG(外部检索替代记忆) 4.分层记忆(短期+长期) 5.向量检索(语义搜索相关历史)。', tags: '上下文,窗口限制', importance: 4 },
    ...
    { category: 'RAG', question: 'Embedding模型怎么选？', answer: '考虑因素：维度(768-1536)、多语言支持、领域适配、推理速度。推荐：OpenAI text-embedding-3-small/large、BGE系列(中文优秀)、E5、Cohere embed。需在自己的数据上评估效果。', tags: 'Embedding,选型', importance: 4 },
  ];
